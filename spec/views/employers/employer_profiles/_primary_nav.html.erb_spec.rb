...
RSpec.describe "employers/employer_profiles/_primary_nav AS BROKER AGENCY STAFF" do
  ...
  let(:broker_agency_staff_role) { FactoryGirl.create(:broker_agency_staff_role, aasm_state: "active", broker_agency_profile_id: broker_agency_profile.id)}
  ...
  it "should display the standard tabs for Employer [broker and employer control]" do
    ...
    expect(rendered).to have_selector('a', text: /my PA Insurance Department/i)
    expect(rendered).to match(/li.*class.*active.*my PA Insurance Department/mi)
