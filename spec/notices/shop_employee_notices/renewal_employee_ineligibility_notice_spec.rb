require 'rails_helper'
 RSpec.describe ShopEmployeeNotices::RenewalEmployeeIneligibilityNotice, :dbclean => :after_each do
  ...
  let(:benefit_sponsorship) { FactoryGirl.build(:benefit_sponsorship, benefit_coverage_periods: [FactoryGirl.build(:benefit_coverage_period, :next_years_open_enrollment_coverage_period)]) }
  let!(:hbx_profile) {FactoryGirl.create(:hbx_profile, benefit_sponsorship: benefit_sponsorship)}
  ...
  let(:application_event){ double("ApplicationEventKind",{
                            ...
                            :title => "Termination of Employer’s Health Coverage Offered Through PA Insurance Department"})
                          }
  let(:valid_params) {{
     ...
     context "invalid params" do
      [:mpi_indicator,:subject,:template].each do  |key|
        it "should NOT initialze with out #{key}" do
          ...
          expect{ShopEmployeeNotices::RenewalEmployeeIneligibilityNotice.new(census_employee, valid_params)}.to raise_error(RuntimeError,"Required params #{key} not present")
        end
      end
    end
  end
